...
export default function WellSwapMinimal() {
  ...
  const handleSellSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await InsuranceAPI.registerInsuranceAsset({
        productName: sellData.productName,
        insuranceCompany: sellData.insuranceCompany,
        category: sellData.category,
        annualPayment: parseFloat(sellData.annualPayment),
        totalPayment: parseFloat(sellData.totalPayment),
        contractDate: sellData.contractDate,
        contractPeriod: sellData.contractPeriod,
        paidPeriod: sellData.paidPeriod,
        status: 'pending'
      }, connectedAccount);

      if (result.success) {
        alert(`🎉 등록 완료!\n자산 ID: ${result.assetId}\n트랜잭션: ${result.transactionHash}`);
        // 폼 리셋
        setSellData({
          productName: '',
          insuranceCompany: '',
  ...
  const handleBuySubmit = async (listing: ListingData) => {
    ...
    setIsLoading(true);
    try {
      const result = await InsuranceAPI.purchaseInsuranceAsset(
        listing.id.toString(),
        connectedAccount
      );

      if (result.success) {
        alert(`🎉 구매 완료!\n트랜잭션 ID: ${result.transactionId}\n블록체인 해시: ${result.transactionHash}`);
      } else {
